...
export class TanentController {
    ...
    async create(req: Request, res: Response, next: NextFunction) {
        ...
        const validator = await createTanentSchema.safeParseAsync({ name, address });
        this.logger.info(`Creating tanent with name: ${name} and address: ${address}`);
        try {
            const tanent = await this.tanentService.create({ name: validator?.data?.name!, address: validator?.data?.address! });
